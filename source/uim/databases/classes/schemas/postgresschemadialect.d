...
class PostgresSchemaDialect : SchemaDialect {
     ...
     */
    array listTablesSql(Json[string] configData = null) {
    ...
    array describeColumnSql(string aTableName, Json[string] configData = null) {
    ...
    string columnSql(TableSchema tableSchema, string aName) {
        ...
        if (
            ...
            (
             ...
             result ~= " VARCHAR";
            if (isSet(someData["length"]) && someData["length"] != "") {
                 result ~= "(" ~ someData["length"] ~ ")";
            }
        }
        $hasCollate = [
