...
class SqlserverCompiler : QueryCompiler {
     ...
     * @param \UIM\Database\ValueBinder aBinder Value binder used to generate parameter placeholder
     */
    protected string _buildInsertFromParts(array someParts, Query compiledQuery, ValueBinder aBinder) {
        ...
        auto someModifiers = _buildModifierPart(aQuery.clause("modifier"), compiledQuery, aBinder);

        return "INSERT%s INTO %s (%s) OUTPUT INSERTED.*"
            .format(someModifiers, aTable, someColumns.join(", "));
    }
     ...
     */
    protected string _buildLimitPart(int limitClause, Query aQuery) {
        if (aQuery.clause("offset").isNull) {
            return "";
        }
        return " FETCH FIRST %d ROWS ONLY".format(limitClause);
    }
