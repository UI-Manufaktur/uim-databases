...
abstract class Driver {
     ...
     */
    protected PDO createPdo(string adsn, IConfigData[string] configData = null) {
        action = fn () : new PDO(
            dsn,
   ...
   bool inTransaction() {
    ...
    string schema() {
        return configuration.data("schema"];
    }
    ...
    string lastInsertId(string atable = null) {
        return to!string(this.getPdo().lastInsertId(aTable));
    }
    ...
    TableISchema newTableSchema(string tableName, array someColumns = []) {
        auto className = configuration.data("tableSchema"] ?? TableSchema.class;
     ...
     * Params:
     * \string amessage Message string or query.
     * @param array context Logging context.
     */
    bool log(string amessage, array context = []) {
        if (this.logger.isNull) {
            return false;
        }
        ...
        this.logger.debug(to!string(loggedQuery, ["query": loggedQuery]));
    ...
    string getRole() {
        return configuration.data("_role"] ?? Connection.ROLE_WRITE;
    }
