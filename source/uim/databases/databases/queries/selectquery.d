...
class SelectQuery : Query, IteratorAggregate {
    ...
    void join(string[] atables, STRINGAA typeNames = [], bool overwrite = false) {
        ...
        aTables.byKeyValue
            each!((aliasT) {
                if (!isArray(aliasT.value)) {
                    aliasT.value = ["table": aliasT.value, "conditions": this.newExpr()];
                }
                if (!isString(aliasT.value["conditions"]) && cast(Closure)aliasT.value["conditions"]) {
                    aliasT.value["conditions"] = aliasT.value["conditions"](this.newExpr(), this);
                }
                if (!(cast(IExpression)aliasT.value["conditions"] )) {
                    aliasT.value["conditions"] = this.newExpr().add(aliasT.value["conditions"], typeNames);
                }
                aliasT.key = isString(aliasT.key) ? aliasT.key : null;
                joins[aliasT.key ?:  anI++] = aliasT.value ~ ["type": JOIN_TYPE_INNER, "alias": aliasT.key];
            });

        _parts["join"] = overwrite 
            ? joins
            : chain(_parts["join"], joins);
